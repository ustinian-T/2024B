...
三个临界值将参数空间划分为检测获益与检测亏损两个区域，企业当前参数恰好落在零件1应检、零件2与成品不应检的区域。各临界结论与表~\ref{tab:q2-optimal}的最优策略完全一致，验证了解的合理性。
...
对最优策略分别以 20 万笔向量化模拟与 1 万笔逐事件模拟估计单位期望利润。向量化模拟的均值误差为 60.18 元，$95\%$ 置信区间为 $[60.07,\ 60.29]$；逐事件模拟均值误差为 60.12 元，$95\%$ 置信区间为 $[59.62,\ 60.62]$。两种模拟下的理论期望利润 60.22 元均落入 $95\%$ 置信区间，且模拟得到的采购、半成品装配、成品装配、调换与拆解等事件次数与理论期望的绝对误差均小于 $0.01$。这表明仿真与解析模型不仅在总利润数值上一致，在内部流程层面亦吻合。收敛过程如图~\ref{fig:q3-mc}所示。
...
图~\ref{fig:q3-phase}以关键参数的联合变化为坐标，用利润差刻画决策边界，可识别两个因素同时变化时的交互作用。全局重新优化得到的主要结构切换点为：调换损失约 47.68 元、成品不合格率约 0.1248、半成品检测成本约 5.30 元，均早于仅比较当前策略局部收益所得的理论阈值（调换损失 60 元、成品不合格率 0.15），原因是全局搜索允许多个决策同时变化，其他策略可能在局部决策尚未达到单点盈亏平衡前就率先超过基准策略。
...
在fixture设计抽样场景下，对问题二六种情形与问题三实例分别在名义点估计、90\% 与 95\% 稳健置信水平下重新优化，结果汇总于表~\ref{tab:q4-q3} 与表~\ref{tab:q4-q2}（见附录）。
所有情形均通过单调性角点检验与 Bellman/枚举回归一致性检验，确认最坏点确实落在各参数同时取上界处。



由表~\ref{tab:q4-q2} 可见，六种情形中有四种（S1、S3、S5、S6）在引入统计不确定性后最优策略发生切换，且切换方向均为增加一道检测：S1、S3、S5 由不检测零件 2 转为检测零件 2，S6 由全不检测转为检测零件 1；而 S2、S4 策略保持不变。这一结果具有明确的经济解释：稳健优化只在决策接近经济临界点时才改变策略，对于已经开启检测或质量风险远离阈值的情形，策略保持稳定。

\begin{table}[H]
\centering
\renewcommand{\arraystretch}{1.4}
\caption{问题三实例的稳健重优化结果（单位：元）}
\label{tab:q4-q3}
\begin{tabularx}{\textwidth}{C C C C}
\toprule
置信水平 & 最优策略 & 期望成本 & 期望利润 \\
\midrule
名义 & 1111111111111101 & 139.78 & 60.22 \\
90\% & 1111111111111111 & 161.30 & 38.70 \\
95\% & 1111111111111111 & 163.25 & 36.75 \\
\bottomrule
\end{tabularx}
\end{table}

问题三在 95\% 稳健水平下唯一变化是开启成品检测（末位由 0 变 1），其余 15 个决策位不变。这是因为前端检测已充分控制质量风险，但承认次品率为抽样估计后成品流入市场的风险被放大，稳健模型选择在出厂环节增设一道检测。

在经济价值分解方面，问题三在 95\% 置信水平下：名义状态采用稳健策略少赚约 \textbf{2.22 元/订单}（PoR），不利情形下较名义策略多保护约 \textbf{2.26 元/订单}（RG），参数恶化造成利润损失 25.74 元（PDE），最坏利润保证 36.75 元（WG）。企业以约 2.22 元日常代价换取 2.26 元额外保护，代价与收益基本对称。

在策略稳定性方面，问题二各情形在 95\% 稳健水平下的策略稳定率 $R_\pi$ 普遍超过 0.93（如 S1 在 $B=512$ 时 $R_\pi=0.943$），S2、S4 达到 1.0；S6 名义水平稳定率约 0.66，建议补充抽样。问题三因序贯抽样存在截尾问题，未强行报告 Bootstrap 稳定率，转以 65536 枚举、4096 角点检验与灵敏度阈值作为稳健性佐证。
...
\textbf{（1）统计层检验：} 对置信上界的覆盖性能进行验证。固定样本 Clopper-Pearson 上界在 $p=0.05, 0.1, 0.2$、$n=20\sim200$ 的网格上，90\% 与 95\% 区间的实际覆盖率均在 $0.907\sim 1.0$ 之间，保守裕度为正，符合名义水平；问题一序贯 e-process 反演上界在 1000 次重复试验下，$p=0.05$ 时覆盖率 1.0，$p=0.2$ 时覆盖率 0.998，均满足置信保证。family-scope 校验显示问题二 $d=3$ 与问题三 $d=12$ 的 Bonferroni 上界与全局 $d=30$ 诊断结果一致（误差量级 $10^{-11}$），系统级同时置信集合构造正确。

\textbf{（2）生产层检验：} 验证前两问评价器在第四问调用下的正确性。问题二对全部 48 个策略格点比较 Bellman 矩阵解与解析闭式解，最大绝对误差 $4.3\times 10^{-14}$；问题三手工核算七项成本分量（采购、零件检测、装配、半成品检测、成品检测、调换、拆解）与程序输出一致，总成本 139.78 元，误差 $2.8\times 10^{-14}$。两项均确认生产层评价器精度无损。

\textbf{（3）稳健层检验：} 验证最坏点简化与策略切换的合理性。利润单调性检验显示，问题二各情形与问题三在 $[\hat{p}_j, U_j]$ 网格上各参数的利润最大上升违反均为 0，确认固定策略利润关于缺陷概率单调非增；角点检验对问题二全部 8 个角点与问题三关键策略的全部 4096 个角点计算实际最坏利润，均与上界角点利润一致，确认最坏点确为各参数同时取上界。相图一致性检验进一步确认策略切换逻辑：$p_F=0.10$ 时成品检测不开启，$p_F=0.20$ 时开启，切换阈值 $p_F^*\approx 0.15$，与解析临界 $b_F/(S\cdot p_F\cdot L)=6/40$ 吻合。
...
\item 抽样方案兼顾判定可靠性与检测经济性，在风险可控的前提下显著降低平均检测次数。
