...
\title{基于序贯抽样与稳健决策的生产优化问题研究}  % 论文标题
...
问题一要求设计一抽样检测方案用于判断是否接收供应商提供的零配件。本文综合采用概率模型与统计检验方法，将检测判定问题转化为可计算的数学模型\textsuperscript{\cite{si2011}}。供应商声称次品率不超过标称值$p=10\%$，企业承担检测成本。
...
假如直接使用二项分布做多次重复判断，每次重新计算会导致出现偶然显著的情况。同时，不能很好地完成题目对检测次数尽可能少的要求。为此，本文引入e-progress方法，建立起证据分数分别记录支持拒收与接收的证据强度\textsuperscript{\cite{ding2026,shen2003}}。
...
问题四在前两问的基础上需要在次品率未知的情况下求解。考虑到抽样误差的影响，需要将统计不确定性纳入生产决策考虑。直接使用点估计可能导致最优方案对参数扰动敏感，鲁棒优化理论表明可在目标性能与决策稳健性之间权衡\textsuperscript{\cite{bertsimas2004}}。
因此，本文首先用置信区间量化不确定性，
...
\begin{enumerate}[itemindent=2em]
    ...
    \hspace{2em}针对小样本条件下的验收问题，可直接基于二项分布构造精确统计检验\textsuperscript{\cite{ding2026}}。
    ...
    \hspace{2em}为减少不必要的检测次数，本文采用序贯决策思想，根据当前累计检验结果动态更新统计证据，并在满足预设判定条件时及时停止检测\textsuperscript{\cite{shen2003}}。
...
本文选择将企业成功交付一件合格成品作为完整决策周期。但由于成品可能在检测环节判断为不合格，也可能因为售后问题被退回，因此不能仅比较单次生产，必须将整个决策周期内的期望总成本作为评价标准。检测本身会产生额外成本，最优检测方案不能仅以降低次品率为目标，还需综合权衡检测成本与质量损失之间的经济关系\textsuperscript{\cite{xue2020}}。
...
问题三具有明显的BOM层级结构\textsuperscript{\cite{wang2001}}，若展开所有节点状态，模型复杂度会迅速增加。因此，本文采用分层状态压缩递推方法，将各子树的成本、质量等关键信息汇总后传递至上层。各阶段质量状态与决策结果形成树形事件演化结构，沿叶节点向根节点递推概率与期望收益\textsuperscript{\cite{liang2026}}。其网络拓扑与状态递推框架如图~\ref{fig:q3-bom}所示。
...
\midrule
\texttt{q1\_model.py} & 问题一：e-process 序贯抽样模型与精确二项检验 \\
\texttt{q2\_model.py} & 问题二：Bellman 生产循环状态方程与 64 策略枚举 \\
\texttt{q3\_model\_refactored.py} & 问题三：BOM 分层递推与 $2^{16}$ 策略全枚举 \\
\texttt{Q4\_main\_model.py} & 问题四：Bonferroni 校正与系统置信集稳健重优化 \\
\midrule
\texttt{generate\_all\_figures.m} & 问题一：五张论文图的一键生成入口 \\
\texttt{q2\_generate\_all\_figures\_v5\_2.m} & 问题二：v5.2 版六图统一生成脚本 \\
\texttt{q3\_generate\_figures.m} & 问题三：图2--图6 严格版独立生成函数 \\
\texttt{generate\_q4\_figures.m} & 问题四：稳健优化论文图表一键生成入口 \\
\midrule
\texttt{Q2\_color.m} & 第二/三问共用自然色板定义函数 \\
\texttt{Q2\_style.m} & 第二/三问共用字体/字号/线宽样式函数 \\
\bottomrule
\end{tabularx}
\label{tab:文件列表}
\end{table}
...
\lstinputlisting[language=Python, caption={问题一核心模型节选}, label={code:q1},
    linerange={49-86, 209-270, 934-962}]
    {2024B 支撑材料/q1_model.py}
\newpage
\section{问题二代码}

\lstinputlisting[language=Python, caption={问题二核心模型节选}, label={code:q2},
    linerange={115-186, 343-470}]
    {2024B 支撑材料/q2_model.py}
\newpage
\section{问题三代码}

\lstinputlisting[language=Python, caption={问题三核心模型节选}, label={code:q3},
    linerange={53-117, 193-290, 491-520}]
    {2024B 支撑材料/q3_model_refactored.py}
\newpage
\section{问题四代码}

\lstinputlisting[language=Python, caption={问题四核心模型节选}, label={code:q4},
    linerange={170-180, 526-672}]
    {2024B 支撑材料/Q4_main_model.py}
